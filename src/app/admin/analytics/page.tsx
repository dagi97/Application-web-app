...
const ErrorDisplay = ({ error }: { error: any }) => (
  <div className="flex justify-center items-center h-screen">
    <div className="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded max-w-md">
      <strong>Error:</strong> {error?.message || 'Failed to load analytics data'}
    </div>
  </div>
);
...
const AnalyticsDashboard = () => {
  ...
  const funnelData = analyticsData ? [
    { name: 'Applied', value: analyticsData.application_funnel.submitted ?? 0 },
    { name: 'Under Review', value: analyticsData.application_funnel.pending_review ?? 0 },
    { name: 'Interview', value: analyticsData.application_funnel.in_progress ?? 0 },
    { name: 'Accepted', value: analyticsData.application_funnel.accepted ?? 0 }
  ] : [];
  ...
  const schoolData = analyticsData
    ? Object.entries(analyticsData.school_distribution || {})
        .map(([name, value]) => ({
          name,
          value,
          percent: analyticsData.total_applicants
            ? (value / analyticsData.total_applicants) * 100
            : 0
        }))
  ...
  const mapData = analyticsData
    ? Object.entries(analyticsData.country_distribution || {})
        .map(([country, count]) => ({ country, value: count ?? 0 }))
    : [];

  const sortedMapData = [...mapData].sort((a, b) => (b.value ?? 0) - (a.value ?? 0));

  if (isLoading) return <LoadingSpinner />;
  if (error || !analyticsData) return <ErrorDisplay error={error || { message: 'No analytics data available' }} />;
  ...
  return (
    ...
    <div className="flex flex-col items-start p-6 md:p-10 gap-8 w-full max-w-[1920px] mx-auto bg-gray-50">
      ...
      <div className="flex flex-col items-start gap-1 w-full max-w-[1280px]">
        <h1 className="text-2xl md:text-3xl font-bold text-gray-900">Application Analytics</h1>
        <p className="text-base text-gray-500">Insights for the G7 November Intake</p>
      </div>
      ...
      <div className="flex flex-col items-start gap-8 w-full max-w-[1280px]">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 w-full">
          <div className="bg-white p-5 rounded-lg shadow-lg border-l-4 border-blue-500">
            ...
            <p className="text-3xl font-semibold text-gray-900 mt-1">
              {formatNumber(analyticsData.total_applicants ?? 0)}
            </p>
          </div>
          ...
          <div className="bg-white p-5 rounded-lg shadow-lg border-l-4 border-green-500">
            ...
            <p className="text-3xl font-semibold text-gray-900 mt-1">
              {(analyticsData.acceptance_rate ?? 0).toFixed(1)}%
            </p>
          </div>
          ...
          <div className="bg-white p-5 rounded-lg shadow-lg border-l-4 border-yellow-500">
            ...
            <p className="text-3xl font-semibold text-gray-900 mt-1">
              {(analyticsData.average_review_time_days ?? 0).toFixed(1)} Days
            </p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 w-full">
          ...
          <div className="bg-white p-6 rounded-lg shadow-lg">
            <h3 className="text-xl font-semibold text-gray-900">Application Funnel</h3>
            <p className="text-sm text-gray-500">Applicant journey from submission to acceptance</p>
            <div className="mt-3 h-96">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart layout="vertical" data={funnelData} margin={{ top: 20, left: 100, right: 20, bottom: 20 }}>
                  <XAxis type="number" />
                  <YAxis type="category" dataKey="name" />
                  <Tooltip formatter={(value) => [formatNumber(Number(value ?? 0)), 'Applicants']} />
                  <Legend />
                  <Bar dataKey="value" name="Applicants" fill="#6C63FF" barSize={30} radius={[5, 5, 5, 5]}>
                    <LabelList dataKey="value" position="right" formatter={(value) => formatNumber(Number(value ?? 0))} />
                  </Bar>
                </BarChart>
              </ResponsiveContainer>
            </div>
                ...
                <PieChart>
                  <Pie
                    ...
                    label={({ name, value, percent }) =>
                      `${name}: ${formatNumber(Number(value ?? 0))} (${(percent ?? 0).toFixed(1)}%)`
                    }
                  >
                  ...
                  <Tooltip formatter={(value, name, props: any) => [
                    `${formatNumber(Number(value ?? 0))} (${(props.payload.percent ?? 0).toFixed(1)}%)`,
                    props.payload.name
                  ]} />
            ...
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={sortedMapData}>
                ...
                <Bar dataKey="value" name="Applicants" radius={[5, 5, 0, 0]}>
                  ...
                  <LabelList dataKey="value" position="top" formatter={(value) => formatNumber(Number(value ?? 0))} />
                </Bar>
              </BarChart>
            </ResponsiveContainer>
          </div>
