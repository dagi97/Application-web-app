...
const ActionCard = ({ title, description, page }: ActionCardProps) => {
  return (
    <div>
      <h3 className="text-[#111827] text-lg md:text-xl font-bold mb-2">{title}</h3>
      ...
      >
        <div className="text-[#4F46E5] flex items-center gap-1">
          <p className="font-bold">Go to {page}</p>
          <Image src="/arrow.svg" alt="Arrow" height={13} width={13} />
        </div>
      </Link>
    </div>
  );
