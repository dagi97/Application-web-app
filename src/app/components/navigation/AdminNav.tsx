...
import { ProfileData } from '@/lib/redux/types/ProfileData';
...
const AdminNav = () => {
    ...
    const userName = profileData?.data?.full_name || "Admin"; // Fallback to 'Admin' if no name is available
        ...
        <nav className='shadow-md bg-white'>
            <div className='flex gap-64 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8'>
                ...
                <ul className='flex gap-10 mt-6 mb-6'>
                    ...
                    <li><Link href='/admin/cycle' className={IsActive('/admin/cycles') ? 'border-b-2 border-[#6366F1] text-[#111827]': 'text-[#6B7280]'}>Cycles</Link></li>
                ...
                <ul className='flex gap-4 mt-6 mb-6'>
                    <li><Link href='/profile' className='text-[#4F46E5]'>Your Profile</Link></li> {/*After you figure out the href add classname so that you can underline */}
                    <li>{userName}</li> 
                    <li><Link href='#' className='text-[#6B7280]'  onClick={() => signOut({
                        callbackUrl: '/'
                    })}>Logout</Link></li>
                </ul>
            </div>
